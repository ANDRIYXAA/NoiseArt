...
- [x] **B1/B2 — ефекти на вектор.** Растеризація фігури (rect/circle/rounded + fill/stroke) на CPU + ланцюжок фільтрів (blend за opacity) → кеш-текстура (`VectorLayerEffect::rasterize`/`applyFilters`). Вектор без фільтрів — примітивами як раніше.
- [x] **B1/B2 — обрізання по формі.** Фото/вектор-діти кола/заокругленого вектора обрізаються по ФОРМІ через альфа-маску (`ClipShape.h` / `applyClipMask`), рахується в `updateProcessing`.
- [x] **B1/B2 — шейдери (фільтри + обрізання).** Шейдер з фільтрами/обрізанням → readback пікселів щокадру + ті самі фільтри + маска форми.
- [x] **B1/B2 — group effects.** Фільтр у групі/артборді каскадно діє на весь контент усередині (успадкування фільтрів вгору по дереву). Окреме видалення будь-якого вузла (`removeLayerPtr`).
- [x] **B1/B2 — текст.** Рендер через NanoVG обраним шрифтом (вибір шрифту полагоджено), зум-чітко (вектор через `nvgScale` + переростеризація при зумі), + фільтри/обрізання. Додавання шрифтів із файлу («Add Font…» → `nvgCreateFont` у рантаймі, динамічний список).
- [ ] **Далі (опц.):** завантаження шрифтів з **URL** (мережа); композит-then-filter замість поелементного; обрізання шейдер/текст по формі вже працює.
